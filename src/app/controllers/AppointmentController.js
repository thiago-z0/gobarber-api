...
class AppointmentController {
  ...
  async store (req,res) {
    ...
    if(!isProvider){
      return res.status(401).json({error: 'You can only create apointments with providers'});
    }

    if(req.userId == provider_id){
      return res.status(401).json({error: 'You are a Provider. Stop to try find a bug in me.'});
    }
  ...
  async delete (req,res) {
    ...
    if (appointment.canceled_at !== null){
      return res.status(401).json({ error: "This appontment is already canceled." });
    }
